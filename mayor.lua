    ...
    if show_main_window.v then
		...
		if imgui.Button(u8'Набор в мэрию №1.', btn_size) then
			lecture = lua_thread.create(function()
				...
				wait(cfg.main.zaderjka)
				sampSendChat("/gov [City Hall]: Так как на оф. портале 'City Hall' открыты заявления на должность Адвоката. Мы ждем тебя!")
			end)
		end	
          ...
          if f then
            ...
            if updateversion ~= thisScript().version then
              lua_thread.create(function(prefix)
                local dlstatus = require('moonloader').download_status
                local color = -1
                mtext((prefix..'Обнаружено обновление. Пытаюсь обновиться c '..thisScript().version..' на '..updateversion), color)
                wait(250)
                downloadUrlToFile(updatelink, thisScript().path,
                  function(id3, status1, p13, p23)
                    ...
                    elseif status1 == dlstatus.STATUS_ENDDOWNLOADDATA then
                      ...
                      mtext((prefix..'Обновление завершено!'), color)
                      goupdatestatus = true
                      lua_thread.create(function() wait(500) thisScript():reload() end)
                    end
                    if status1 == dlstatus.STATUSEX_ENDDOWNLOAD then
                      if goupdatestatus == nil then
                        mtext((prefix..'Обновление прошло неудачно. Запускаю устаревшую версию..'), color)
                        update = false
                      end
                    end
                  end
